...
pod_b_ip=$(oc get pod -n netproj-b-${user} $(oc get pod -n netproj-b-${user} | grep -v deploy | awk '/ose-/ {print $1}') -o jsonpath='{.status.podIP}{"\n"}')
echo $pod_b_ip


echo -n "Getting Pod A's Name... "
pod_a_name=$(oc get pod -n netproj-a-${user} | grep -v deploy | awk '/ose-/ {print $1}')
echo $pod_a_name
...
while [ $i -le ${max_tries} ]; do
  ...
  if oc exec -n netproj-a-${user} $pod_a_name -- timeout 2 bash -c "</dev/tcp/$pod_b_ip/5000" 2>/dev/null ; then
    break
  fi
